...
class MyPushServiceHandler : public example::PushServiceCobSvIf {
public:
    MyPushServiceHandler() {}
    virtual ~MyPushServiceHandler() {}
    virtual void push(const std::string& data) {
        std::cout << "recv push msg : " << data << std::endl;
    }
};
...
int main(int argc,  char* argv[]) {
    ...
    // 注册rpc服务，用于接收反向RPC请求
    MyPushServiceHandler my_service;
    client.GetPebbleRpc(pebble::kPEBBLE_RPC_BINARY)->AddService(&my_service);
