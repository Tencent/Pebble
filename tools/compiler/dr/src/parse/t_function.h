...
class t_function : public t_doc {
 public:
  ...
  std::string get_timeout_ms() {
    std::string timeoutms("-1");
    std::string timeout_annotations[2][2] = {
        { "timeout",   " * 1000" }, // 单位为s
        { "timeoutms", ""        }, // 单位为ms
    };
    for (int i = 0; i < 2; i++) {
        std::map<std::string, std::string>::iterator it = annotations_.find(timeout_annotations[i][0]);
        ...
        bool invalid = false;
        for (int j = 0; j < static_cast<int>(value.length()); j++) {
        ...
        timeoutms.assign("(");
        timeoutms.append(value);
        timeoutms.append(timeout_annotations[i][1]);
        timeoutms.append(")");
        break;
    }
