...
%type<tattrs>    Attributes
%type<ttype>     Attribute
%type<ttype>     AttributePairs
%type<tannot>    AttributePair
%type<id>        AttributeValue
...
Struct:
  Attributes StructHead tok_identifier XsdAll '{' FieldList '}' TypeAnnotations
    {
      pdebug("Struct -> tok_struct tok_identifier { FieldList }");
      validate_simple_identifier( $3);
      $6->set_xsd_all($4);
      $6->set_union($2 == struct_is_union);
      $$ = $6;
      $$->set_name($3);
      if ($8 != NULL) {
...
Field:
  Attributes CaptureDocText FieldIdentifier FieldRequiredness FieldType FieldReference tok_identifier FieldValue XsdOptional XsdNillable XsdAttributes TypeAnnotations CommaOrSemicolonOptional
    {
  ...
  Attributes Attribute
    {
      if ($1 != NULL) {
          ...
          $$ = new t_attributes(g_program);
      }
      if (!$$->set_attribute($2->get_name(), $2->annotations_)) {
          yyerror("attribute[%s] redefine!", $2->get_name().c_str());
          exit(1);
      }
    }
|
...
Attribute:
  '[' tok_identifier '(' AttributePairs ')' ']'
    {
      validate_simple_identifier($2);
      if ($4->annotations_.empty()) {
  ...
  AttributePairs ',' AttributePair
    {
      pdebug("TypeAnnotationList -> TypeAnnotationList , TypeAnnotation");
      $$ = $1;
      if ($$->annotations_.find($3->key) != $$->annotations_.end()) {
          yyerror("attribute key[%s] redefine!", $3->key.c_str());
          exit(1);
      }
