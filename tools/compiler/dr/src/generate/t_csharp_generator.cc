...
class t_csharp_generator : public t_oop_generator
{
  public:
    ...
    std::string function_signature_async_begin_ex(t_function* tfunction, std::string prefix = "");
...
void t_csharp_generator::generate_csharp_struct_reader(ofstream& out, t_struct* tstruct) {
  ...
  out << indent() << "if (field.ID == -1) {" << endl;
  indent_up();
  for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
      out << indent() <<(f_iter == fields.begin() ? "" : "else ") <<
          "if (field.Name == \"" << (*f_iter)->get_name() << "\") {" << endl;
      out << indent(1) << "field.ID = " << (*f_iter)->get_key() << ";" << endl;
      out << indent() << "}" << endl;
  }
  indent_down();
  out << indent() << "}" << endl;
  ...
  indent(out) <<
    "public void Read (byte[] buff)" << endl;
  indent(out) << "{" << endl;
  indent_up();

  indent(out) << "Read(new TBinaryProtocol(new TMemoryBuffer(buff)));" << endl;
  ...
  indent(out) <<
    "public void FromJson (string str)" << endl;
  indent(out) << "{" << endl;
  indent_up();

  indent(out) << "Read(new TJSONProtocol(new TMemoryBuffer(Encoding.UTF8.GetBytes(str))));" << endl;
...
void t_csharp_generator::generate_csharp_struct_writer(ofstream& out, t_struct* tstruct) {
  ...
  indent(out) <<
    "public void Write (byte[] buff)" << endl;
  indent(out) << "{" << endl;
  indent_up();

  indent(out) << "Write(new TBinaryProtocol(new TMemoryBuffer(buff)));" << endl;
  ...
  indent(out) <<
    "public string ToJson ()" << endl;
  indent(out) << "{" << endl;
  indent_up();

  indent(out) << "var buff = new TMemoryBuffer();" << endl;

  indent(out) << "Write(new TJSONProtocol(buff));" << endl;
  indent(out) << "return Encoding.UTF8.GetString(buff.GetBuffer());" << endl;
...
void t_csharp_generator::generate_service_client(t_service* tservice) {
  ...
  indent(f_service_) <<
    "public class Client : " << extends_client << " {" << endl;
  indent_up();
  ...
  f_service_ << indent() << "service_name_ = \"" << tservice->get_name() << "\";" << endl << endl;
  scope_down(f_service_);
  ...
  if (extends.empty()) {
    f_service_ <<
      ...
      indent() << "protected long seqid_;" << endl <<  
      indent() << "protected string service_name_;" << endl << endl;
    ...
    f_service_ << indent() << "public string ServiceName" << endl;
    scope_up(f_service_);
    indent(f_service_) << "get { return service_name_; } set {service_name_ = value;}" << endl;
    scope_down(f_service_);
    f_service_ << endl << endl;
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    if (!(*f_iter)->is_oneway()) {
        string returntype = type_name((*f_iter)->get_returntype());
        // Concurrent_
        indent(f_service_) <<
          "public " << function_signature_async_begin_ex(*f_iter) << endl;
        scope_up(f_service_);
        indent(f_service_) << 
            "var ars = new AsyncRpcSessoin((ex) => {" << endl;
        indent_up();
        indent(f_service_) << "if (ex != null) {" << endl;
        f_service_ << indent(1) << "callback(ex" << (returntype == "void" ? "": (", default("+returntype+")")) <<");" << endl;
        f_service_ << indent(1) << "return 0;" << endl;
        ...
        if (!(*f_iter)->get_returntype()->is_void()) {
          if (nullable_) {
            if (type_can_be_null((*f_iter)->get_returntype())) {
              f_service_ <<
                ...
                indent(1) << "callback(null, result.Success);" << endl;
            } else {
              f_service_ <<
                indent() << "if (result.Success.HasValue) {" << endl <<
                indent(1) << "callback(null, result.Success.Value);" << endl;
            }
          } else {
            f_service_ <<
          ...
          for (x_iter = xceptions.begin(); x_iter != xceptions.end(); ++x_iter) {
            if (nullable_) {
              f_service_ <<
                indent() << "} else if (result." << prop_name(*x_iter) << " != null) {" << endl <<
                indent(1) << "  callback(result." << prop_name(*x_iter)  << (returntype == "void" ? "": (", default("+returntype+")")) << ");" << endl;
            } else {
              f_service_ <<
                indent() << "} else if (result.__isset." << normalize_name((*x_iter)->get_name()) << ") {" << endl <<
                indent(1) << "  callback(result." << prop_name(*x_iter) << (returntype == "void" ? "": (", default("+returntype+")")) << ");" << endl;
            }
          }
          ...
          f_service_ << indent(1) <<
            "callback(new TApplicationException(TApplicationException.ExceptionType.MissingResult, \""
            << funname << " failed: unknown result\")" << (returntype == "void" ? "": (", default("+returntype+")")) << ");"
            << endl;
          f_service_ << indent() << "}" << endl;

        }

        f_service_ << indent() << "return 0;" << endl;
        indent_down();
        indent(f_service_) <<  "}, " << (*f_iter)->get_timeout_ms() << ");" << endl << endl;

        indent(f_service_) <<  "long sid = Rpc.Instance.AddNewSession (ars);" << endl << endl;

        f_service_ <<
          indent() << "oprot_.WriteMessageBegin(new TMessage(\"" << service_name_ << ":" << funname << "\", " <<
        ...
        indent(f_service_) << "oprot_.Transport.ServiceName = service_name_;" << endl;
...
void t_csharp_generator::generate_service_server(t_service* tservice) {
  ...
  // 增加获取servicename接口
  indent(f_service_) << (tservice->get_extends() == NULL ? "virtual" : "override") << " public string ServiceName()";
  f_service_ << endl;
  scope_up(f_service_);
  f_service_ << indent() << "return \"" << service_name_ << "\";"<< endl;
  scope_down(f_service_);
  f_service_ << endl << endl;

  if (extends.empty()) {
    f_service_ <<
      indent() << "protected delegate void ProcessFunction(long seqid, TProtocol iprot, TProtocol oprot);" << endl;
  }
...
void t_csharp_generator::generate_process_function(t_service* tservice, t_function* tfunction) {
  (void) tservice;
  indent(f_service_) <<
    "public void " << tfunction->get_name() << "_Process(long seqid, TProtocol iprot, TProtocol oprot)" << endl;
  scope_up(f_service_);
  ...
  f_service_ <<
    indent() << "oprot.WriteMessageBegin(new TMessage(\"" << service_name_ << ":" << tfunction->get_name() << "\", TMessageType.Reply, seqid)); " << endl <<
...
void t_csharp_generator::generate_deserialize_field(ofstream& out, t_field* tfield, string prefix, bool is_propertyless) {
  ...
  if (type->is_struct() || type->is_xception()) {
    ...
    else if (type->is_base_type() && type->annotations_.find("u") != type->annotations_.end()) {
    ...
    if (type->is_base_type()) {
      ...
      switch(tbase) {
        ...
        case t_base_type::TYPE_BOOL:
          ...
          break;
        case t_base_type::TYPE_BYTE:
          out << "WriteByte((sbyte)" << nullable_name << ");";
          break;
        case t_base_type::TYPE_I16:
          out << "WriteI16((short)" << nullable_name << ");";
          break;
        case t_base_type::TYPE_I32:
          out << "WriteI32((int)" << nullable_name << ");";
          break;
        case t_base_type::TYPE_I64:
          out << "WriteI64((long)" << nullable_name << ");";
          break;
        case t_base_type::TYPE_DOUBLE:
...
string t_csharp_generator::base_type_name(t_base_type* tbase, bool in_container, bool in_param, bool is_required) {
  ...
  bool is_unsigned = tbase->annotations_.find("u") != tbase->annotations_.end();
  switch (tbase->get_base()) {
    ...
    case t_base_type::TYPE_BOOL:
      return "bool" + postfix;
    case t_base_type::TYPE_BYTE:
      return (is_unsigned ? "byte" : "sbyte") + postfix;
    case t_base_type::TYPE_I16:
      return (is_unsigned ? "ushort" : "short") + postfix;
    case t_base_type::TYPE_I32:
      return (is_unsigned ? "uint" : "int") + postfix;
    case t_base_type::TYPE_I64:
      return (is_unsigned ? "ulong" : "long") + postfix;
    case t_base_type::TYPE_DOUBLE:
...
string t_csharp_generator::function_signature_async_begin_ex(t_function* tfunction, string prefix) {
  string comma = (tfunction->get_arglist()->get_members().size() > 0 ? ", " : "");
  return "void " + normalize_name(prefix + tfunction->get_name()) + "(" + argument_list(tfunction->get_arglist()) + comma + "Action<Exception" +
      (tfunction->get_returntype()->is_void() ? "" : (", " + type_name(tfunction->get_returntype()))) + "> callback)";
}
