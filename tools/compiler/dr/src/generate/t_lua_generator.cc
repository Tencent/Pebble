...
void t_lua_generator::init_generator() {
  ...
  if (gen_requires_) {
    ...
    for (size_t i = 0; i < includes.size(); ++i) {
        f_types_ << endl << "require '" << get_namespace(includes[i]) << "ttypes'";
    }
  }
}
  ...
  indent(out) << "while true do" << endl;
    ...
    indent(out) << "if ftype == TType.STOP then" << endl;
      ...
      for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
        indent_down();
        indent(out)
          << "elseif fid == " << (*f_iter)->get_key() << " or (fid == -1 and fname == '"<< (*f_iter)->get_name() << "') then" << endl;
          indent_up();
          indent(out)
            ...
            // Read field contents
            generate_deserialize_field(out, *f_iter, "obj.");
...
void t_lua_generator::generate_lua_struct_writer(ofstream& out,
  ...
  // function
  indent(out) << endl << endl
    << "function " << tstruct->get_name() << ".write(oprot, obj)" << endl;
  indent_up();

    indent(out)
      << "oprot:writeStructBegin('" << tstruct->get_name() << "')" << endl;
    for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
      indent(out) << "if obj." << (*f_iter)->get_name() << " then" << endl;
      indent_up();
        indent(out)
        ...
        generate_serialize_field(out, *f_iter, "obj.");
...
void t_lua_generator::generate_service(t_service* tservice) {
  ...
  f_service_ << endl << "local getseqid , getthreadid, wait_reply = rpc.seqid, threadpool.running, rpc.wait_reply " << endl;
  ...
  f_service_ << endl << "rpc.registerType('" << tservice->get_name() <<"', " << tservice->get_name() << "Client, " << tservice->get_name() << "Processor";
  if (tservice->get_extends() != NULL) {
	  f_service_ << ", '" << tservice->get_extends()->get_name() << "'";
  }
  f_service_ <<")" << endl;
...
void t_lua_generator::generate_service_client(ofstream &out,
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    indent(out) << endl << "function " << classname << ":" << sig << endl;
      ...
      indent(out) << "local seqid = self:send_" << sig << endl << indent();
      if (!(*f_iter)->is_oneway()) {
		out << "wait_reply(seqid, 3)" << endl << indent();
    ...
    indent(out) << endl << "function " << classname << ":send_" << sig << endl;
	  ...
	  indent(out) << "local seqid = getseqid()" << endl;
      indent(out) << "self.oprot:writeMessageBegin('" << tservice->get_name() << ":" <<funcname << "', "
                  << ((*f_iter)->is_oneway() ? "MessageType.ONEWAY" : "MessageType.CALL")
                  << ", seqid, getthreadid())" << endl;
      indent(out) << "local args = {}" << endl;
      ...
      indent(out) << funcname << "_args.write(self.oprot, args)" << endl;
      indent(out) << "self.oprot:writeMessageEnd()" << endl;
      indent(out) << "self.otrans:writeEnd()" << endl;
      indent(out) << "self.otrans:flush()" << endl;
	  if (!(*f_iter)->is_oneway()) {
		indent(out) << "return seqid" << endl;
	  }
    ...
    if (!(*f_iter)->is_oneway()) {
      indent(out)
        << endl << "function " << classname << ":recv_" << (*f_iter)->get_name() <<"()" << endl;
        indent_up();

        out <<
          indent() << "local result = " << funcname << "_result.read(self.iprot)" << endl <<
          indent() << "self.iprot:readMessageEnd()" << endl <<
		  indent() << "self.itrans:readEnd()" << endl;
        ...
        if (!(*f_iter)->get_returntype()->is_void()) {
          ...
          out <<
            indent() << "end" << endl <<
            indent() << "error(Exception.new(Exception.MISSING_RESULT))" << endl;
        }
...
void t_lua_generator::generate_process_function(ofstream &out,
  ...
  indent(out) << endl << "function " << classname << ".process_" << fn_name
    << "(handler, seqid, threadid, iprot, oprot, itrans, otrans)" << endl;
    indent_up();

    // Read the request
    out <<
      indent() << "local args = " << argsname << ".read(iprot)" << endl <<
      indent() << "iprot:readMessageEnd()" << endl <<
	  indent() << "itrans:readEnd()" << endl <<
      indent() << "local status, res = pcall(handler." << fn_name
               << ", handler";
	...
	if (!tfunction->is_oneway()) {
		...
		out <<  indent() << "if not status " << (xf.size() > 0 ? "and not Exception.isCustom(res) " : "") <<"then" << endl <<
		  indent() << "  oprot:writeMessageBegin('" << tservice->get_name() << ":"<<fn_name << "', MessageType.EXCEPTION, seqid, threadid)" << endl <<
		  indent() << "  Exception.write(oprot, res)"
				   << endl;
		...
		out <<
		  indent() << "else" << endl <<
		  indent() << "  oprot:writeMessageBegin('" << tservice->get_name() << ":"<<fn_name << "', MessageType.REPLY, seqid, threadid)" << endl <<
		  indent() << "  local result = {}" << endl ;
		  
		if(xf.size() > 0) {
			out <<  
			  indent() << "  if status then" << endl <<
			  indent() << "    result.success = res" << endl <<
			  indent() << "  else" << endl <<
			  indent() << "    result.ouch = res" << endl <<
			  indent() << "  end" << endl ;
		} else {
			out <<  "  result.success = res" << endl;
		}
		out <<
		  indent() << "  "<< resultname <<".write(oprot, result)" << endl <<
		  indent() << "end" << endl <<
...
void t_lua_generator::generate_deserialize_struct(ofstream &out,
                                                  ...
                                                  string prefix) {
  indent(out)
    << prefix << " = " << tstruct->get_name() << ".read(iprot)" << endl;
}
...
void t_lua_generator::generate_serialize_struct(ofstream &out,
                                                t_struct* tstruct,
                                                string prefix) {
  indent(out) << tstruct->get_name() << ".write(oprot, " << prefix <<")" << endl;
}
