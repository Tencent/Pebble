 ...
 */
void version() {
  printf("Pebble %s %s %s\n", pebble::PebbleVersion::GetVersion().c_str(), __TIME__, __DATE__);
}
 ...
 */
void usage() {
  fprintf(stderr, "Usage: pebble [options] file\n\n");
  fprintf(stderr, "Use pebble -help for a list of options\n");
  exit(1);
}
...
void help() {
  ...
  fprintf(stderr, "  --allow-neg-keys  Allow negative field keys (Used to "
          "preserve protocol\n");
  fprintf(stderr, "                compatibility with older .pebble files)\n");
