...
class t_cpp_generator : public t_oop_generator {
 public:
    ...
    : t_oop_generator(program)
  {
    ...
    iter = parsed_options.find("client");
    gen_client_code = (iter != parsed_options.end());
 ...
 private:
  ...
  /**
   * Generate code for terminal(default generate for server).
   */
  bool gen_client_code;
...
void t_cpp_generator::init_generator() {
  ...
  if (!gen_client_code) {
    f_types_ <<
      "#include <rpc/common/reflection.h>" << endl <<
      endl;

    f_types_ <<
      "#include <rpc/common/field_pack.h>" << endl <<
      endl;
  }
 ...
 * Generates a class that holds all the constants.
 */
void t_cpp_generator::generate_consts_in_services(std::vector<t_const*> consts) {
...
void t_cpp_generator::generate_cpp_struct(t_struct* tstruct, bool is_exception) {
  ...
  if (!gen_client_code) {
    generate_struct_reflection_info(f_types_impl_, tstruct);
  }
}
...
void t_cpp_generator::generate_struct_declaration(ofstream& out,
  ...
  if (reflection && !gen_client_code) {
    out << indent() << "static const pebble::rpc::reflection::TypeInfo* S_GetTypeInfo();" << endl << endl;
    out << indent() << "virtual const pebble::rpc::reflection::TypeInfo* GetTypeInfo();" << endl << endl;
    out << "private:" << endl << endl;
    out << indent() << "static pebble::rpc::reflection::TypeInfo* s_type_info;" << endl << endl;

    for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
        out << indent() << "class FieldInfo_" << (*m_iter)->get_name() << " : public ::pebble::rpc::reflection::FieldInfo {" << endl;
        out << indent() << "public:" << endl;
        indent_up();
        out << indent() << "FieldInfo_" << (*m_iter)->get_name() << "(std::string field_name, int16_t field_id,"
            << "pebble::rpc::protocol::TType field_type,"
            << "bool is_optional, ::pebble::rpc::reflection::AttributesType attributes) " << endl;
...
void t_cpp_generator::generate_service(t_service* tservice) {
  ...
  if (!gen_client_code) {
    f_header_ <<
      indent() << "class Rpc;" << endl <<
      indent() << "class ConnectionObj;" << endl;
  } else {
    f_header_ <<
...
void t_cpp_generator::generate_service_client(t_service* tservice, string style) {
  ...
  if (style != "Cob") {
    if (!gen_client_code) {
      // gen server code
      f_header_ <<
        indent() << "/// @brief 使用指定服务地址和编码协议构造客户端对象，适用场景描述：" << endl <<
        indent() << "///        1. 按名字访问服务: 不需要填任何参数" << endl <<
        indent() << "///        2. 广播: 需要指定service_url参数，格式为broadcast://channel_name，可选择是否指定protocol_type参数" << endl <<
        indent() << "///        3. 按指定地址访问服务: 需要指定service_url参数，可选择是否指定protocol_type参数" << endl <<
        indent() << "/// @param service_url 服务地址，默认为空，为空时按服务名获取服务地址" << endl <<
        indent() << "///    服务地址支持tcp/http/tbus/zk/broadcast协议，如" << endl <<
        indent() << "///        tcp://127.0.0.1:6666" << endl <<
        indent() << "///        http://127.0.0.1:7777" << endl <<
        indent() << "///        tbus://1.1.1:8888" << endl <<
        indent() << "///        zk://127.0.0.1:1888" << endl <<
        indent() << "///        broadcast://channel_name" << endl <<
        indent() << "/// @param protocol_type 编码协议，默认为BINARY，按服务名寻址时采用服务配置的编码类型" << endl <<
        indent() << "/// @note  特殊连接参数可通过Set接口配置" << endl <<
        indent() << "/// @note  多个地址间用\',\'分隔，zk和broadcast协议不支持多个" << endl <<
        indent() << service_name_ << style << "Client" << short_suffix <<
        "(" << struct_para << ");" << endl << endl;
 
     f_header_ <<
       indent() << "/// @brief 使用连接对象构造客户端对象，仅适用反向RPC场景" << endl <<
       indent() << "/// @note  使用连接对象构造客户端对象后使用Set接口配置参数将不生效" << endl <<
       indent() << service_name_ << style << "Client" << short_suffix <<
       "(" << struct_para_1 << ");" << endl << endl;
    } else {
      // gen client code
      f_header_ <<
       indent() << service_name_ << style << "Client" << short_suffix <<
    ...
    if (!gen_client_code && extends.empty()) {
      f_header_ << indent() << "/// @brief 设置编码协议类型，支持binary/json/bson" << endl;
      f_header_ << indent() << "void SetProtocolType(pebble::rpc::PROTOCOL_TYPE protocol_type);" << endl << endl;
      ...
      f_header_ << indent() << "/// @brief 设置目标分区id" << endl;
      f_header_ << indent() << "/// @note  目标分区必须为公共分区" << endl;
      f_header_ << indent() << "void SetTargetUnit(int unit_id);" << endl << endl;

      f_header_ << indent() << "/// @brief 设置路由key" << endl;
      f_header_ << indent() << "void SetRouteKey(uint64_t route_key);" << endl << endl;
    }
  ...
  if (extends.empty()) {
    ...
    if (!gen_client_code) {
      f_header_ << indent() << "void get_protocol();" << endl << endl;
    }
    ...
    if (!gen_client_code) {
      f_header_ << indent() << "::pebble::rpc::Rpc* client_;"  << endl;
    } else {
    ...
    if (!gen_client_code) {
      f_header_ << endl;
      f_header_ << indent() << "std::string service_address;"  << endl;
      f_header_ << indent() << "std::string service_name;"  << endl;
      f_header_ << indent() << "int prot_type;"  << endl;
      f_header_ << indent() << "int unit_id;"  << endl;
      f_header_ << indent() << "bool cross_unit;"  << endl;
      f_header_ << indent() << "uint64_t route_key;"  << endl;
    }
  ...
  if (!gen_client_code) {
    out <<
      scope << service_name_ << style << "Client" << short_suffix << "(" << struct_para_a << ") ";
  
    if (extends.empty()) {
      out <<  "{" << endl;
      out << indent(1) << "prot_owner_ = true;" << endl;
      out << indent(1) << "service_address = service_url;" << endl;
      out << indent(1) << "service_name = \"" << service_name_ << "\";" << endl;
      out << indent(1) << "prot_type = protocol_type;" << endl;
      out << indent(1) << "cross_unit = false;" << endl;
      out << indent(1) << "unit_id = -1;" << endl;
      out << indent(1) << "route_key = 0;" << endl;
      out << indent() << "}" << endl;
    } else {
      out <<  ":" << endl;
      out <<
        indent(1) << extends << style << client_suffix <<
        "(service_url, protocol_type) {" << endl <<
        "}" << endl;
    }
    out << endl;
    ...
    if (extends.empty()) {
      out <<  "{" << endl;
      out <<
      indent(1) << "prot_owner_ = false;" << endl <<
      indent(1) << "service_name = \"" << service_name_ << "\";" << endl <<
      indent(1) << "cross_unit = false;" << endl <<
      indent(1) << "unit_id = -1;" << endl <<
      indent(1) << "route_key = 0;" << endl <<
      indent(1) << "client_ = pebble::rpc::Rpc::Instance();" << endl <<
      indent(1) << "setProtocol" << short_suffix 
      << "(client_->GetProtocol(conn_obj));" << endl <<
        indent() << "}" << endl;
    } else {
      out <<  ":" << endl;
      out <<
        indent(1) << extends << style << client_suffix <<
        "(conn_obj) {" << endl <<
        //"  std::string service_name(\""<< service_name_ <<"\");" << endl << 
        //"  client_->SetServiceName2Protocol(piprot_, service_name);" << endl << 
        "}" << endl;
    }
    out << endl;
  } else {
    ...
    if (extends.empty()) {
      ...
      out <<
        indent(1) << extends << style << client_suffix <<
        "(rpc_connector) {" << endl << "}" << endl;
    }
    out << endl;
  }

  if (!gen_client_code && extends.empty()) {
    out << "void " << scope << "SetProtocolType(pebble::rpc::PROTOCOL_TYPE protocol_type)" << endl;
    out << "{" << endl;
    out << indent(1) << "prot_type = protocol_type;" << endl;
    out << "}" << endl << endl;
    ...
    out << "void " << scope << "SetTargetUnit(int unit_id)" << endl;
    out << "{" << endl;
    out << indent(1) << "cross_unit = true;" << endl;
    out << indent(1) << "this->unit_id = unit_id;" << endl;
    out << "}" << endl << endl;
  
    out << "void " << scope << "SetRouteKey(uint64_t route_key)" << endl;
    out << "{" << endl;
    out << indent(1) << "this->route_key = route_key;" << endl;
    out << indent(1) << "if (poprot_) {" << endl;
    out << indent(2) << "poprot_->setRouteKey(route_key);" << endl;
    out << indent(1) << "}" << endl;
    out << "}" << endl << endl;
  }
  ...
  if (!gen_client_code && extends.empty()) {
    out << "void " << scope << "get_protocol()" << endl;
    out << "{" << endl;
    out << indent(1) << "if (!prot_owner_) {" << endl;
    out << indent(2) << "return;" << endl;
    out << indent(1) << "}" << endl << endl;
    
    out << indent(1) << "if (piprot_) {" << endl;
    out << indent(2) << "return;" << endl;
    out << indent(1) << "}" << endl << endl;

    out << indent(1) << "client_ = pebble::rpc::Rpc::Instance();" << endl;
    out << indent(1) << "if (service_address.empty()) {" << endl;
    out << indent(2) << "service_address = client_->GetZooKeeperAddress();" << endl;
    out << indent(1) << "}" << endl << endl;
  
    out << indent(1) << "if (!cross_unit) {" << endl;
    out << indent(2) << "setProtocol(client_->GetProtocol(service_address, service_name, static_cast<pebble::rpc::PROTOCOL_TYPE>(prot_type)));" << endl;
    out << indent(1) << "} else {" << endl;
    out << indent(2) << "setProtocol(client_->GetProtocol(service_address, service_name, unit_id));" << endl;
    out << indent(1) << "};" << endl << endl;
    out << indent(1) << "if (poprot_) {" << endl;
    out << indent(2) << "poprot_->setRouteKey(route_key);" << endl;
    out << indent(1) << "}" << endl;
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    if (true) {
      ...
      if (gen_client_code) {
        out << indent() << _this << "oprot_->getTransport()->setServiceName(\"" << service_name_ << "\");" << endl;
      }
      out <<
      ...
      if (!(*f_iter)->is_oneway()) {
        ...
        std::string cob_inter_err("cob(pebble::rpc::ErrorInfo::kOtherException);");
        if (!(*f_iter)->get_returntype()->is_void()) {
          t_field returnfield((*f_iter)->get_returntype(), "_return");
          out <<
            indent() << declare_field(&returnfield, true) << endl;
          ...
          cob_inter_err = "cob(pebble::rpc::ErrorInfo::kOtherException, _return);";
        }
        ...
        out << indent() << "if (!prot) {" << endl;
        out << indent(1) << cob_inter_err << endl;
        out << indent(1) << "throw ::pebble::rpc::TApplicationException(::pebble::rpc::TApplicationException::INTERNAL_ERROR, \"protocol is null\");" << endl;
        out << indent() << "}" << endl;
  ...
  // Cob style.
    ...
    if (!tfunction->is_oneway()) {
      ...
      out <<
        ...
        indent() << "}" << endl << endl;
      out <<
        indent() << "uint32_t bytes = 0;" << endl <<
        indent() << "try {" << endl <<
        indent(1) << "oprot->writeMessageBegin(\"" << 
          service_name_ << ":" << tfunction->get_name() <<
          "\", ::pebble::rpc::protocol::T_REPLY, seqid);" << endl <<
        indent(1) << "result.write(oprot);" << endl <<
        indent(1) << "oprot->writeMessageEnd();" << endl;
        if (gen_client_code) {
          out << indent(1) << "oprot->getTransport()->setServiceName(\"" << service_name_ << "\");" << endl;
        }
      out <<
        indent(1) << "bytes = oprot->getTransport()->writeEnd();" <<
          endl <<
        indent(1) << "oprot->getTransport()->flush();" << endl <<
        indent() << "} catch (pebble::rpc::TException ex) {" << endl <<
        indent(1) << "return cob(false);" << endl <<
        indent() << "}" << endl << endl;
      out <<
