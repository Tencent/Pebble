...
#ifndef PEBBLE_RPC_GROUP_PROTOCOL_H
#define PEBBLE_RPC_GROUP_PROTOCOL_H
...
class GroupProtocol : public TProtocol
{
public:
    ...
    void addProtocol(cxx::shared_ptr<TProtocol> prot);

    int rmvProtocol(cxx::shared_ptr<TProtocol> prot);

    std::vector< cxx::shared_ptr<TProtocol> > getProtocols();
    ...
    virtual uint32_t readMessageBegin_virt(std::string& name, TMessageType& messageType, int64_t& seqid) {
        selectProtocol();
...
protected:
    cxx::shared_ptr<TProtocol> protocol;
    std::vector< cxx::shared_ptr<TProtocol> > m_protocols;
    int m_route_type;
...
#endif // PEBBLE_RPC_GROUP_PROTOCOL_H
