...
#include <stdint.h>
#include "source/rpc/protocol/dense_protocol.h"
...
const int TDenseProtocol::FP_PREFIX_LEN =
  pebble::rpc::reflection::local::FP_PREFIX_LEN;
...
inline uint32_t TDenseProtocol::subWriteI64(const int64_t i64) {
  return vlqWrite(i64);
}
...
uint32_t TDenseProtocol::subReadI64(int64_t& i64) {
  uint64_t u64;
  uint32_t rv = vlqRead(u64);
