...
#include "source/rpc/protocol/debug_protocol.h"
...
uint32_t TDebugProtocol::writeListBegin(const TType elemType,
  ...
  oss << size;
  bsize += writePlain(
      "list<" + fieldTypeName(elemType) + ">[" + oss.str() + "] {\n");
  indentUp();
...
uint32_t TDebugProtocol::writeSetBegin(const TType elemType,
  ...
  oss << size;
  bsize += writePlain(
      "set<" + fieldTypeName(elemType) + ">[" + oss.str() + "] {\n");
  indentUp();
