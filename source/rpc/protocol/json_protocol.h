...
#ifndef PEBBLE_RPC_PROTOCOL_JSONPROTOCOL_H
#define PEBBLE_RPC_PROTOCOL_JSONPROTOCOL_H
...
class TJSONProtocol : public TVirtualProtocol<TJSONProtocol> {
 ...
 private:
  // 框架中protocol复用，message begin时需要清空context防止异常
  void clearContext();
...
std::string PebbleJSONString(const PebbleStruct& ts) {
    using namespace pebble::rpc::transport;
    using namespace pebble::rpc::protocol;
...
#endif // PEBBLE_RPC_PROTOCOL_JSONPROTOCOL_H
