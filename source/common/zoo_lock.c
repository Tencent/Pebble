    ...
    if (mutex->id != NULL) {
        ...
        while (ret == ZCONNECTIONLOSS && (count < 3)) {
            ...
            if (ret == ZCONNECTIONLOSS) {
                PLOG_DEBUG(("connectionloss while deleting the node"));
                nanosleep(&ts, 0);
                count++;
            }
        }
...
static char* child_floor(char **sorted_data, int len, char *element) {
    ...
    for (i = 0; i < len; i++) {
        if (strcmp(sorted_data[i], element) < 0) {
            ret = sorted_data[i];
        } else {  // add by alexdu. I have found at least one, need not to contiune.
            break;
        }
...
static int retry_getchildren(zhandle_t *zh, char* path, struct String_vector *vector, 
    ...
    while (ret == ZCONNECTIONLOSS && count < retry) {
        ...
        if (ret == ZCONNECTIONLOSS) {
            PLOG_DEBUG(("connection loss to the server"));
            nanosleep(ts, 0);
            count++;
        }
    }
...
static int retry_zoowexists(zhandle_t *zh, char* path, watcher_fn watcher, void* ctx,
    ...
    while (ret == ZCONNECTIONLOSS && count < retry) {
        ...
        if (ret == ZCONNECTIONLOSS) {
            PLOG_DEBUG(("connectionloss while setting watch on my predecessor"));
            nanosleep(ts, 0);
            count++;
        }
    }
        ...
        if (mutex->id != NULL) {
            ...
            if (lessthanme != NULL) {
                ...
                if (strcmp(mutex->id, owner_id) == 0) {
                    PLOG_DEBUG(("got the zoo lock owner - %s", mutex->id));
                    mutex->isOwner = 1;
