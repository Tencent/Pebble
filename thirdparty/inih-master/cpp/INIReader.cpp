...
#include <utility>
#include <string>
#include "thirdparty/inih-master/ini.h"
using std::string;
...
int32_t INIReader::GetInteger(string section, string name,
...
int INIReader::ValueHandler(void* user, const char* section, const char* name,
    ...
    std::map<std::string, std::set<std::string>*>::iterator fieldSetIt = reader
            ->_fields.find(sectionKey);
    if (fieldSetIt == reader->_fields.end()) {
        fieldsSet = new std::set<std::string>();
        reader->_fields.insert(
                std::pair<std::string, std::set<std::string>*>(sectionKey,
                                                               fieldsSet));
    } else {
