...
public:
	...
	*/
    ...
    //added bu funnyren 201400326 数组引用时也可以使用字符串类型的下标获取
    bool RapidJsonCheckIsAllNumber(const Ch* name)
    {
        if (NULL == name)
        {
            return false;
	...
	GenericValue& operator[](const Ch* name) {
        //added by funnyren to support acquire array by string index like:<index>
	    if (RapidJsonCheckIsAllNumber(name))
	    {
			SizeType index = (SizeType)strtoull(name + 1, NULL, 10);
	        //RAPIDJSON_ASSERT(IsArray());
			//RAPIDJSON_ASSERT(index < data_.a.size);
			if (!IsArray() || index >= data_.a.size)
			{
