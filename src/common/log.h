...
public:
    ...
    static void SetMaxFileSize(uint32_t max_size_Mbytes);

    /// @brief 设置日志文件滚动个数
    /// @note 默认滚动个数为10
    static void SetMaxRollNum(uint32_t num);
    ...
    static void RegisterLogWriteFunc(const LogWriteFunc& tlog_func);
...
#define PLOG_FATAL(fmt, ...) pebble::Log::Write(pebble::LOG_PRIORITY_FATAL, __FILE__, __LINE__, __FUNCTION__, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_ERROR(fmt, ...) pebble::Log::Write(pebble::LOG_PRIORITY_ERROR, __FILE__, __LINE__, __FUNCTION__, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_INFO(fmt,  ...) pebble::Log::Write(pebble::LOG_PRIORITY_INFO,  __FILE__, __LINE__, __FUNCTION__, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_DEBUG(fmt, ...) pebble::Log::Write(pebble::LOG_PRIORITY_DEBUG, __FILE__, __LINE__, __FUNCTION__, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_TRACE(fmt, ...) pebble::Log::Write(pebble::LOG_PRIORITY_TRACE, __FILE__, __LINE__, __FUNCTION__, fmt, ##__VA_ARGS__); // NOLINT
